...
const Board = (props: BoardProps) => {
	...
	const onDragEnd = (result: any) => {
		...
		const sourceCol = draggableData!.columns!.find(
			(i: any) => i.id == source.droppableId
		)
		const destinationCol = draggableData!.columns!.find(
			(i: any) => i.id == destination.droppableId
		)
		...
		dispatch(
			dragDropTasks({
				...
				newBoard: newBoard,
				newTask: task,
				newColId: newStartEnd.newEndCol.id,
			})
		)
		setDraggableData(newBoard)
	}
	...
	if (userId?.length === 0) {
		return (
			<div className={`Board ${onHide} Board--noItem`}>
				<p className="Board--noItem__txt">Please Log In to get started with</p>
